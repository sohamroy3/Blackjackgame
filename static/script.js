...
'cards':['2','3','4','5','6','7','8','9','10','K','J','Q','A'],//card array
...
const YOU=blackjackGame['you']//cons because not really going to change
...
//queryselector will select the query button
...
function blackjackHit()
{
    if(blackjackGame['isStand']===false)
    {
        let card=randomCard();
        console.log(card);
        showCard(card,YOU);//YOU is an argument.That is nothing but active player
...
function randomCard()
{
    let randomIndex=Math.floor(Math.random()*13);//13 possibility are there
...
function showCard(card,activePlayer)
{
    if(activePlayer['score'] <= 21)
    {
        ...
        document.querySelector(activePlayer['div']).appendChild(cardImage);//activeplayer because it will work dynamically.
        //activeplayer is an argument that is passed to the function
        hitSound.play();
    }
}
...
function blackjackDeal()
{
    if(blackjackGame['turnsOver']===true)
    {
        ...
        let yourImages=document.querySelector('#your-box').querySelectorAll('img');//# is used as an convention that the element is in HTML page not any variable of javascript
        let dealerImages=document.querySelector('#dealer-box').querySelectorAll('img');//List of all images
